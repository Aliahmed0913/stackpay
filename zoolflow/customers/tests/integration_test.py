...
class TestCustomerProfileAPIView:
    ...
    def test_update_profile(
        self, api_client, payload, excepected_status, authenticate_client
    ):
        ...
        assert (
            customer.is_verified == original_verified
        )  # The field (is_verified) is immutable by anyone except the admin
...
class TestAddressViewSet:
    def test_valid_addresses(self, api_client, authenticate_client):
        ...
        assert (
            api_client.post(url, data=payload).status_code
            == status.HTTP_400_BAD_REQUEST
        )
        ...
        last_address = (
            Address.objects.filter(customer=customer).order_by("-created_at").first()
        )
